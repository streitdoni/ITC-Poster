...
communication in MANETs is challenging task because the participants rely on limited hardware and 
...
Transmitting huge amouts of data in Mobile Ad-Hoc Networks (MANET) is a challenging task due i.e. to the shared medium, dynamic changing topology and arising bottlenecks. Especially video or audio streams are subject to certain Quality of Service (QoS) requirements. Voice over IP (VoIP) for instance relies on a delay less than $250ms$ and a video stream should not have a packet loss rate more than $5\%$. MANETs are network structures without a central infrastructure. Hence, every participating node acts as router and forwards packets towards destinations. Consequently, a route through the topology is established depending on the constellation of nodes.\\
Several reactive and proactive routing protocols exist which implement individual path discovery processes. AODV broadcasts the whole notwork with the objective to find the fastest route towards the requested destination. Typically, the received RREP proposes the route with least intermediate nodes and each relay usually has low network utilization. Another reactive routing protocol is DSR. The route discovery process OLSR maintains   and routing protocols e.g. AODV can be assumed, that the proposed route is  leads top Utilization, available slot time are beside the position of the node caracteristics of  of the shared medium are tilization, Beside the position of a 
\section{Ease of Use}
